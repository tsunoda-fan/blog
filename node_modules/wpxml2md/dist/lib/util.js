 ...
 * @type {Array}
 */
const BlockElements = ['address', 'article', 'aside', 'audio', 'blockquote', 'body', 'canvas', 'center', 'dd', 'dir', 'div', 'dl', 'dt', 'fieldset', 'figcaption', 'figure', 'footer', 'form', 'frameset', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'header', 'hgroup', 'hr', 'html', 'isindex', 'li', 'main', 'menu', 'nav', 'noframes', 'noscript', 'ol', 'output', 'p', 'pre', 'section', 'table', 'tbody', 'td', 'tfoot', 'th', 'thead', 'tr', 'ul'];
/**
 * Elements of void.
 * @type {Array.<String>}
 */
...
class Util {
  ...
  static existsSync(path) {
    try {
      _fs.default.accessSync(_path.default.resolve(path), _fs.default.F_OK);
  ...
  static formatDate(date, format) {
    ...
    f = f.replace(/ss/g, ('0' + s).slice(-2)); // Single digit
  ...
  static mkdirSync(path) {
    const dir = _path.default.resolve(path);
  ...
  static uniquePathWithSequentialNumber(path, min = 1, max = 256) {
    const originalPath = _path.default.resolve(path);
    ...
    const base = _path.default.basename(originalPath, ext);

    const parent = _path.default.dirname(originalPath);

    for (let i = min; i <= max; ++i) {
      const name = base + '-' + i + ext;

      const uniquePath = _path.default.join(parent, name);
