 ...
 */
const MarkdownConverters = [// Paragraph
{
  filter: 'p',
...
}, // Block quote
{
  filter: 'blockquote',
  replacement: (node, content) => {
    let result = _util.default.trim(content);
...
}, // List item
{
  ...
  replacement: (node, content) => {
    ...
    const index = _util.default.arrayIndexOf(parent.children, node) + 1;
    const ol = /ol/i;
...
{
  filter: node => {
    return _util.default.isBlockElement(node);
  },
  replacement: (node, content) => {
    return '\n\n' + _util.default.outerHTML(node, content) + '\n\n';
  }
}, // Anything else!
{
  filter: () => {
  ...
  replacement: (node, content) => {
    return _util.default.outerHTML(node, content);
  }
}];
var _default = MarkdownConverters;
