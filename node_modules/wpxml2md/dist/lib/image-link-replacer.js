...
const RequestGet = _util.default.promisify(_request.default);
/**
...
const enumFiles = dir => {
  ...
  items.forEach(item => {
    const path = _path.default.join(dir, item);
...
const downloadImages = async (images, dir, logger) => {
  ...
  for (let image of images) {
    ...
    try {
      ...
      _fs.default.writeFileSync(_path.default.join(dir, image.fileName), body, 'binary');
...
const parseImageLink = (markdown, basename) => {
  ...
  return {
    links,
    images: urls.filter((url, i, arr) => arr.indexOf(url) === i).map((url, i) => {
      return {
        url,
        fileName: `${basename}-${i + 1}${_path.default.extname(url)}`
      };
    })
  };
};
...
const replaceLinks = (links, images) => {
  ...
  for (let link of links) {
    ...
    for (let image of images) {
      const regexp = new RegExp(_util2.default.escapeRegExp(image.url), 'g');
...
var _default = ImageLinkReplacer;
